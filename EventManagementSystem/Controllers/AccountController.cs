    ...
    public class AccountController : Controller
    {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    if (user != null)
                    {
                        ...
                        {
                            ViewBag.errorMessage = "Email is not confirmed";
                            return View("Error");
                        }
                    }
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    if (user.Email != null)
                    {
                        ...
                        if (roles.Contains("Event Planner"))
                        {
                            return RedirectToAction("Index", "Dashboard");
                        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.Email != null)
                {
                    var user = new ApplicationUser { UserName = model.UserName.ToLower(), Email = model.Email, Name = model.Name, PhoneNumber = model.Contact };
                    var result = await UserManager.CreateAsync(user, model.Password);
                    if (result.Succeeded)
                    {
                        await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                        ...
                        client.Phone = model.Contact;
                        client.UserName = model.UserName.ToLower();
                ...
                {
                    var user = new ApplicationUser { UserName = model.UserName.ToLower(), Name = model.Name, PhoneNumber = model.Contact };
                    var result = await UserManager.CreateAsync(user, model.Password);
                    if (result.Succeeded)
                    {
                        await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
